"""Read-only probe for mower weather and rain-protection settings."""
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Read-only Dreame mower weather/rain protection probe."
    )
    parser.add_argument(
        "--include-raw",
        action="store_true",
        help="Include raw CFG/RPET app-action responses.",
    )
    parser.add_argument(
        "--device-index",
        type=int,
...
async def main() -> None:
    ...
    try:
        ...
        result = await client.async_get_weather_protection(
