"""Read-only probe for mower-native app mowing preferences."""
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Read-only Dreame mower preference probe using app actions."
    )
    parser.add_argument(
        "--include-raw",
        action="store_true",
        help="Include raw PREI/PRE app-action responses.",
    )
    parser.add_argument(
        "--map-indices",
        default="",
...
async def main() -> None:
    ...
    try:
        ...
        result = await client.async_get_mowing_preferences(
